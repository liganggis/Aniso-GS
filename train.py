...
from gaussian_renderer import render, perprocess
import sys
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, load_iteration, debug_from):
    ...
    nums_threshold = train_img_nums * 0.2

    start_ema = opt.iterations - train_img_nums * 2

    testing_iterations = [opt.iterations - 20*i for i in range(nums)]
    saving_iterations = [opt.iterations]
    if load_iteration == -1:
        saving_iterations.append(start_ema)
        testing_iterations.append(7000)
        saving_iterations.append(7000)
    ...
    for iteration in range(first_iter, opt.iterations + 1):   
        ...
        if iteration % 1000 == 1:
            with torch.no_grad():
                obs_cnt = torch.zeros_like(gaussians.get_opacity)
                for view in scene.getTrainCameras():
                    render_pkg_tmp = perprocess(view, gaussians, pipe, bg, offset=dataset.offset, isContracting=dataset.contract, isTraning=True)
                    out_obs = render_pkg_tmp["visibility_filter"]
                    obs_cnt[out_obs > 0] += 1
                prune_mask = ((obs_cnt) < nums_threshold).squeeze()
                gaussians.get_vmask = prune_mask
                ratio = torch.sum(prune_mask) / prune_mask.shape[0]
                if ratio < 0.3:
                    gaussians.get_vmask = (torch.zeros_like(gaussians.get_opacity) > 1).squeeze()
        ...
        if iteration < opt.densify_until_iter:
            render_pkg = render(viewpoint_cam, gaussians, pipe, bg, vmask=gaussians.get_vmask, offset=dataset.offset, isContracting=dataset.contract, isTraning=True)
        else:
            render_pkg = render(viewpoint_cam, gaussians, pipe, bg, vmask=gaussians.get_vmask, offset=dataset.offset, isContracting=dataset.contract, isTraning=False)
        ...
        with torch.no_grad():
            ...
            # Log and save
            result_test = training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, gaussians.ema, render, (pipe, background, gaussians.get_vmask, dataset.offset, dataset.contract, False))
