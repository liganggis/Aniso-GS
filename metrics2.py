...
def render_set(model_path, name, isUsingEMA, offset, contract, iteration, views, gaussians, pipeline, background):
    ...
    #     for view in views:
    #         render_pkg_tmp = perprocess(view, gaussians, pipeline, background, offset=offset, isContracting=contract)
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        rendering = torch.clamp(render(view, gaussians, pipeline, background, vmask=gaussians.get_vmask, offset=offset, isContracting=contract)["render"], 0.0, 1.0)
    ...
    iter_start.record()
    for view in views:
        render(view, gaussians, pipeline, background, vmask=gaussians.get_vmask, offset=offset, isContracting=contract)["render"]
    # end_time = time.time()
    iter_end.record()
