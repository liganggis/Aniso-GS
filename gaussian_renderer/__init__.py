...
# 图像渲染主函数
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, vmask = None, offset = 0.3, isContracting=False, isTraning = False):
    """
    Render the scene. 
    ...
    if vmask is None:
        vmask = torch.ones_like(mask)

    # mask0 = mask & (~vmask) # matmul
    # mask1 = mask & vmask # concat
    # concat
    # mask1 = mask
    ...
    if isContracting:
        temPos0 = contract(pc.get_xyz[mask0].detach())
        temPos1 = contract(pc.get_xyz[mask1].detach())
    else:
        temPos0 = pc.get_xyz[mask0].detach()
        temPos1 = pc.get_xyz[mask1].detach()
    ...
    dir_pp0 = (temPos0 - viewpoint_camera.camera_center)
    dir_pp1 = (temPos1 - viewpoint_camera.camera_center)
    # dir_dist = dir_pp.norm(dim=1, keepdim=True)
    # dir_pp_normalized = dir_pp / dir_dist
    colors0, colors1, opacity0, opacity1 = pc._nlgs(temPos0, temPos1, dir_pp0, dir_pp1, pc.active_sh_degree + 1)
    
    colors_precomps[mask0] = colors0
    colors_precomps[mask1] = colors1
    opacities[mask0] = opacity0
    # opacities[mask1] = opacity1
    if isTraning:
        pc.get_opacity[mask0] = opacity0.detach()
        # pc.get_opacity[mask1] = opacity1.detach()
...
# 图像渲染主函数
def perprocess(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, offset = 0.3, isContracting=False, isTraning = False):
    """
    Render the scene. 
    ...
    _, _, radii, _ = rasterizer.preprocess(
        means3D=means3D,
        scales=scales,
        rotations=rotations,
    )

    return {"visibility_filter" : radii > 0}
