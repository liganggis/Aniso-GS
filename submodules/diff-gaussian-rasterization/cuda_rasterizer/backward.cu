	...
	float* dL_dcov)
{
	...
	// float3 dL_dconic = { dL_dconics[4 * idx], dL_dconics[4 * idx + 1], dL_dconics[4 * idx + 2] };
	float3 dL_dconic = dL_dconics[idx];
	// dL_dopacity[idx] = dL_dconics[4 * idx + 3];
	...
	float a = cov2D[0][0] += offset;
	float b = cov2D[0][1];
	float c = cov2D[1][1] += offset;
...
__device__ void computeCov3D(int idx, const glm::vec3 scale, float mod, const glm::vec4 rot, const float* dL_dcov3Ds, glm::vec3* dL_dscales, glm::vec4* dL_drots)
{
	...
	glm::mat3 S = glm::mat3(0.0000001f);
	...
	glm::vec4* dL_drot)
{
	...
	const float3 dL_dmean2DT = dL_dmean2D[idx];
	dL_dmean.x = (proj[0] * m_w - proj[3] * mul1) * dL_dmean2DT.x + (proj[1] * m_w - proj[3] * mul2) * dL_dmean2DT.y;
	dL_dmean.y = (proj[4] * m_w - proj[7] * mul1) * dL_dmean2DT.x + (proj[5] * m_w - proj[7] * mul2) * dL_dmean2DT.y;
	dL_dmean.z = (proj[8] * m_w - proj[11] * mul1) * dL_dmean2DT.x + (proj[9] * m_w - proj[11] * mul2) * dL_dmean2DT.y;
	...
	float* __restrict__ dL_dcolors)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float3 con_o = collected_conics[j];
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				// dL_drgbs[ch] = dchannel_dcolor * dL_dchannel;
